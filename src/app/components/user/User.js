...
function User() {
  ...
  const retrieveFollowingNumber = () => {
    FollowingDataService.getFollowingCount({ userId: id })
      .then(response =>
  ...
  const retrieveFollowersNumber = () => {
    FollowingDataService.getFollowersCount({ followingId: id })
      .then(response =>
  ...
  const retrieveMoodsNumber = () => {
    MoodDataService.countMoods({ userId: id })
      .then(response =>
      ...
      <>
        <Card sx={{ marginTop: '20px', borderRadius: '5px' }} elevation={0}>
          <Grid container alignItems={'center'}>
            ...
            <Grid onClick={() => navigate(`/user/${id}`)} item xs style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer' }}>
              <Skeleton variant="circular" width={20} height={20} />
              <Skeleton variant="text" width={40}/>
              <Skeleton variant="text" width={10}/>
            </Grid>
            <Grid onClick={() => navigate(`/user/${id}/followers`)} item xs style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer' }}>
              <Skeleton variant="circular" width={20} height={20} />
              <Skeleton variant="text" width={60}/>
              <Skeleton variant="text" width={10}/>
            </Grid>
            <Grid onClick={() => navigate(`/user/${id}/following`)} item xs style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', cursor: 'pointer' }}>
              <Skeleton variant="circular" width={20} height={20} />
              <Skeleton variant="text" width={60}/>
              <Skeleton variant="text" width={10}/>
            </Grid>
